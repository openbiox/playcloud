...
-	超文本标记语言是用来描述和定义网页内容的标记语言，是构成网页的最基本的东西。所谓超文本，就是说它除了能标记文本，还能标记其他的内容，比如：图片，链接，音频，视频等。HTML是宽松的，不严格的web语言，哪怕大小写混用，标签不闭合，浏览器都能以自己的方式读取（有可能误解），不会出现报错。

XML（Extensible Markup Language）

-	可扩展标记语言，是用来存储数据结构的语言，就是给一个文档加一堆标签，说明每段文字是干什么的，有什么意义。这样做的目的是方便存储、传输、分享数据，人和计算机都可以很方便的阅读。XML 和 HTML 有一个明显的区别就是：HTML 的标签都是预定义的，你不可以自己随便增加，比如你不能自造一个标签叫 <openbiox>, 但是 XML 可以。此外XML是严格的语言，因为如果书写的标签不规范，存储数据就会出问题。

XHTML（Extensible Hypertext Markup Language）

-	可扩展超文本标记语言，HTML和XML 一结合，就产生了 XHTML，XHTML就是以XML的语法形式来写HTML。它类似于HTML语言，不过它的语法是严格的，书写更加规范。它的来源是这样的：由于HTML语言不严格，比如大小可以混用，属性值随便你加不加引号，单引号还是双引号也随便你，犯一些小错误都没有关系，这对严谨的程序员来说是一件不可容忍的事，HTML 标准的制定者W3C组织就参考了XML的严谨的优点，将HTML严格规范化，推出了XHTML语言。所以也可以把XHTML理解为HTML的严格语法形式，除此之外，其它方面基本一样。
...
Document 的方法 getElementById() 返回一个匹配特定 ID的元素引用。由于元素的 ID 在大部分情况下要求是独一无二的，这个方法是一个高效查找特定元素的方法。
...
- Undefined 类型只有一个值，即特殊的undefined。如果在声明变量时未对其加以初始化，这个变量的值就是 undefined。
...
- Object类型是一种十分复杂的数据类型，是一组数据和功能的集合。对象可以通过执行 new 操作符后跟要创建 的对象类型的名称来创建。而创建 Object 类型的实例并为其添加属性和（或）方法，就可以创建自定义对象。
...
- boolean 类型：Javascript 有 true 和 false。Python有 True 和 False。它们除了大小写没有什么区别。
...
- String 类型：Javascript 采用UTF16编码。Python 使用 ASCII 码。需要调用 encode、decode 来进行编码转换。使用u作为前缀可以指定字符串使用 Unicode 编码。

- 列表：Javascript 内置了 array 类型（array 也是 object）。Python的列表（List）和Javascript的Array比较接近，而元组（Tuple）可以理解为不可改变的列表。

- 字典、哈希表、对象：Javascript 中大量的使用`{}`来创建对象，这些对象和字典没有什么区别，可以使用`[]`或者.来访问对象的成员。可以动态的添加，修改和删除成员。可以认为对象就是 Javascript 的字典或者哈希表。对象的 key 必须是字符串。Python 内置了哈希表（dictS），和 Javascript 不同的是，dictS 可以有各种类型的 key 值。
...
后端是在后台工作的，控制着前端的内容，主要负责程序设计架构思想，管理数据库等。后端更多的是与数据库进行交互以处理相应的业务逻辑，需要考虑的是如何实现功能、数据的存取、平台的稳定性与性能等。后端使用的语言主要有 PHP、ASP、JAVA等等。
...
举个栗子：假设你是一个餐饮业者，打算做披萨生意。你可以从头到尾，自己生产披萨，但是这样比较麻烦，需要准备的东西多，因此你决定外包一部分工作，采用他人的服务。你有三个方案。1.LaaS: 他人提供厨房、炉子、煤气，你使用这些基础设施，来烤你的披萨。2.PaaS: 除了基础设施，他人还提供披萨饼皮。你只要把自己的配料洒在饼皮上，让他帮你烤出来就行了。也就是说，你要做的就是设计披萨的味道（海鲜披萨或者鸡肉披萨），他人提供平台服务，让你把自己的设计实现。3.SaaS: 他人直接做好了披萨，不用你的介入，到手的就是一个成品。你要做的就是把它卖出去，最多再包装一下，印上你自己的 Logo。
